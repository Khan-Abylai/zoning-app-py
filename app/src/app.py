...
class App(object):
        ...
        for facility in self.config.get_facilities():
            for camera in facility.get_cameras():
                camera_worker = CameraWorker(
                    worker_config=camera, facility_id=facility.get_facility_id()
                )
    ...
    def get_license_plates(self, prediction, snapshot_id):
        if len(prediction) != 0:
                ...
                if data is None:
                    ...
                    if len(frames_paths) == 0:
                        ...
                        for image, snapshot_id in zip(images, snapshot_ids):
                            prediction = self.detection_engine.make_prediction(
                                image=image, camera_ip=frame_data.get_camera_ip(),
                                object_id=frame_data.get_facility_id()
                            )
                            license_plates = self.get_license_plates(prediction, snapshot_id)
